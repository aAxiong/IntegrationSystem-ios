...
var wsUrl = 'ws://123.207.239.73:2346';
             ...
             if (msg.status == 100) {
              ...
              // PRO.GetOneTable(); //每秒发送请求
                ...
                for (var i = 0; i < room.length; i++) {      
                    $('<li rid="' + room[i].id + '" level = "' + room[i].level + '"><span>' + room[i].level + f + (room[i].id).slice(2) + q + '</span></li>').appendTo(IRoom).click(function () {
...
PRO.CheckPoints = function () { //查分
    PRO.Requeset("checkpoints",0 ,{name:user.id, room: user.room, tableNo: user.tableNum }, function (msg) {
        if (msg.status == 100) {
            ...
            f3.eq(1).find("span.lb").text(msg.lost_integral);
            f3.eq(2).find("span.lb").text(msg.success_integral);
                ...
                faulimg.eq(i).transition({"opacity":"1"},200,function(){  
                 ...
                 if(i>=NUM-1){
                     if(COUNT == 1 ){                     //   开始发牌
                        ...
                        heldArray = selected(poker.pkcode);
                        PRO.Selheld(heldArray);
                        //console.log(poker.pkcode+"！开牌，牌型："+heldArray+",后台返回倍数:"+table.type);
                        ...
                        heldArray = selected(poker.pkcode);
                        PRO.Selheld(heldArray); 
                     //   console.log(poker.pkcode+"！换牌，牌型："+heldArray+",后台返回倍数:"+table.type);
                     //   $(".tishi").text(poker.pkcode+"！换牌，牌型："+heldArray+",后台返回倍数:"+table.type);
                        if(poker.jp == 1){
