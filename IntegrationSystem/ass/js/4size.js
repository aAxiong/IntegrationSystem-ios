...
PRO.Requeset3 = function (url,time, data, fn,fncomplete) {
    $.ajax({
        url: 'http://123.207.239.73/fpjadmin/admin.php/proxy/' + url,
        type: "post",
        timeout:time,
        data: data,
        success: function (r) {
...
PRO.Requeset2 = function (url,time, data, fn,fncomplete) {
    $.ajax({
        url: 'http://123.207.239.73/fpjadmin/admin.php/admin/' + url,
        type: "post",
        timeout:time,
        data: data,
        success: function (r) {
...
PRO.Requeset6 = function (url,time, data, fn,fncomplete) {
    var b = "";
    url == "version" ? b = "http://123.207.239.73/fpj/index.php/Home/Redis/"+url : b = 'http://123.207.239.73/fpj/index.php/Home/Game/'+url+'?token='+user.token;
...
PRO.Prompt  = function(CONTENT){ 
    if(KCount){
      ...
      var op = 0;
      if(arg != undefined) {  html = '<div class="prompt hg">'+CONTENT+'<span class="dotting"></span><div class="prok" style="position:fixed;top:0;left:0;width:100%;height:100%;opacity:0;background:#000;z-index:999;"></div></div>'; op = 1;   };
      if($(".prompt").length==0){
